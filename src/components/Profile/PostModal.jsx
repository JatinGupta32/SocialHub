  ...
  return (
    ...
    >
      ...
      >
        ...
        {activePhoto < post.photos.length-1 && <FaCircleChevronRight onClick={()=>setActivePhoto(activePhoto+1)} size={23} className="absolute z-10 opacity-60 translate-x-[43.5vw] translate-y-[25vw] hover:opacity-90 cursor-pointer"/>}
        {activePhoto > 0 && <FaCircleChevronLeft onClick={()=>setActivePhoto(activePhoto-1)} size={23} className="absolute z-10 opacity-60 translate-x-[1vw] translate-y-[25vw] hover:opacity-90 cursor-pointer"/>}
            ...
            
            {
              post.comments.map((comment,i)=>(
                <div className="flex w-full items-center space-x-4 py-2">
                    <img src={comment?.user?.image ? comment?.user?.image : `https://api.dicebear.com/5.x/initials/svg?seed=${comment?.user?.fullname}`} className="w-9 h-9 mb-auto rounded-full object-cover cursor-pointer"></img>
                    <div className="text-sm w-fit">
                      <span className="font-bold cursor-pointer hover:brightness-50">{comment?.user?.username}</span>
                      <span>&nbsp;</span>
                      {comment?.statement}
                    </div>
                </div>
